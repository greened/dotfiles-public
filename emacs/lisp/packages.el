...
(use-package notmuch
  ;; Load notmuch's elisp from the system site-lisp (it must version-match the
  ;; notmuch CLI); fall back to the elpaca copy on hosts without it.
  :ensure t
...
(use-package lsp-mode
  ...
  ;;   * remote monolith files (TRAMP) -> the Cerebras overlay's
  ;;     clangd-monolith-remote client, which uses the shared remote index
  ;;     (see dotfiles-overlays/cerebras/emacs/cerebras.el).
...
(defun change-smtp ()
  ...
  ;; Overlays extend mail config via `(with-eval-after-load 'mail ...)'.
  ;; The former mail.el satisfied that by being a file named mail.el; now
  ;; that it is gone, provide the `mail' marker explicitly (before overlays
  ;; load) so those hooks still fire.
  (provide 'mail))
...
;;                    :ids ((github . "sasha-cb") (confluence . "557058:abc-123")))))
;;   (my/project-people)
